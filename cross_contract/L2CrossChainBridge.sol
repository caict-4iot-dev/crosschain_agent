...
contract L2CrossChainBridge {
    ...
    function SetTimeout(uint256 _timeout) public onlyOwner {
        require(_timeout > 30*60*1000*1000 && _timeout < 180*3600*1000*1000, "SetTimeout: _timeout is not in the range of 30 minutes to 180 hours");
        timeout_ = _timeout;
    ...
    function StartTx(address src_bid, address dest_bid, uint256 amount) public payable {
        ...
        bytes32 txId = sha256(abi.encode(src_bid, dest_bid, chainId_, 0, amount, block.timestamp));
    ...
    function SendAckedTx(bytes memory cross_tx_id, uint8 result, string memory error_desc, uint256 execute_time) public onlyOwner {
        ...
        if (result == 1) {
            ...
            if (crossTxInfo.srcChainId == chainId_) {
