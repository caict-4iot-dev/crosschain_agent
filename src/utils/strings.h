...
namespace utils {
	...
	public:
		...
		static bool ParseUrl(const std::string &strUrl,
			...
			size_t nProtocolPos = strUrl.find(":");
			size_t nHostPos = 0;
			if (std::string::npos != nProtocolPos) {
				bool bValidProtocol = true;
				for (size_t n = 0; n < nProtocolPos; n++) {
					if (!isalnum(strUrl[n])) {
						bValidProtocol = false;
						break;
					}
				}

				if (bValidProtocol) {
					std::string temp_url = strUrl.substr(0, nProtocolPos);
					strProtocol = ToLower(temp_url);
					nHostPos = nProtocolPos + 1;

					while (nHostPos < strUrl.size() && '/' == strUrl[nHostPos]) {
						nHostPos++;
					}
				}
			}
			...
			//for ipv6 compatible
			size_t nPortFindPos = nHostPos;
			size_t nMayIpv6HostEnd = strUrl.find("]", nHostPos);
			if (nMayIpv6HostEnd != std::string::npos) {
				nPortFindPos = nMayIpv6HostEnd;
			}

			size_t nPortPos = strUrl.find(":", nPortFindPos);
			size_t nPathPos = strUrl.find("/", nHostPos);
			if (nPortPos > nPathPos) {
				// maybe such url http://server/about:blank
				nPortPos = std::string::npos;
			}

			if (std::string::npos != nPortPos) {
				strHost = strUrl.substr(nHostPos, nPortPos - nHostPos);
				nPort = Stoui(strUrl.substr(nPortPos + 1));
			}

			if (std::string::npos == nPathPos) {
				strFile = "/";
				if (strHost.size() == 0) {
					strHost = strUrl.substr(nHostPos);
				}
			}
			else {
				strFile = strUrl.substr(nPathPos);
				if (strHost.size() == 0) {
					strHost = strUrl.substr(nHostPos, nPathPos - nHostPos);
				}
			}

			size_t nFilePos = strFile.rfind('/', strFile.find('?', 0));
			if (nFilePos == std::string::npos || nFilePos == 0) {
				strPath = "/";
			}
			else {
				strPath = strFile.substr(0, nFilePos);
			}
			...
			NormalizeUrlPath(strPath);
			NormalizeUrlPath(strFile);

			return true;
		}

		static int NormalizeUrlPath(std::string &strPath) {
			int nCount = 0;

			size_t nNameEndPos = strPath.find('?');
			size_t nAnchorPos = strPath.find('#');
			size_t nProtocolPos = strPath.find("://");

			if (nProtocolPos > nNameEndPos || nProtocolPos > nAnchorPos) {
				nProtocolPos = std::string::npos;
			}

			size_t nValidPos = 0;
			if (nProtocolPos > 0) {
				nValidPos = nProtocolPos + 3;
				while (nValidPos < strPath.size() && strPath[nValidPos] != '/') {
					nValidPos++;
				}
			}

			//such url as:http://bj.nuomi.com?lf=2 => http://bj.nuomi.com/?lf=2
			if (nNameEndPos < nValidPos && std::string::npos != nProtocolPos) {
				std::string strDomain = strPath.substr(0, nNameEndPos);
				std::string strPathTemp = strPath.substr(nNameEndPos, strPath.size());
				strPath = strDomain + "/" + strPathTemp;
				nValidPos = nNameEndPos;
				nNameEndPos = strPath.find('?');
				nAnchorPos = strPath.find('#');
			}

			size_t nMarkPos = strPath.find("/./", nValidPos);
			while (nMarkPos != std::string::npos && nMarkPos < nNameEndPos && nMarkPos < nAnchorPos) {
				strPath.erase(nMarkPos, 2);
				nMarkPos = strPath.find("/./", nValidPos);
				nNameEndPos = strPath.find('?');
				nAnchorPos = strPath.find('#');
				nCount++;
			}
			...
			nAnchorPos = strPath.find('#');
			while (nMarkPos != std::string::npos && nMarkPos < nNameEndPos && nMarkPos < nAnchorPos) {
				size_t nEndPos = nMarkPos + 4;
				size_t nBeginPos = nMarkPos > nValidPos ? (nMarkPos - 1) : nValidPos;
				while (nBeginPos > nValidPos && strPath[nBeginPos] != '/') {
					nBeginPos--;
				}

				strPath.erase(nBeginPos, nEndPos - nBeginPos - 1);
				nMarkPos = strPath.find("/../", nValidPos);
				nNameEndPos = strPath.find('?');
				nAnchorPos = strPath.find('#');
				nCount++;
	...
	static std::string HexStringToBin(const std::string &hex_string, bool force_little = false){
		...
		static std::string Bin2ToHexString(const std::string &value) {
			std::string result;
			for (size_t i = 0; i < value.size() && i < 2; i++) {
				result = AppendFormat(result, "%02x", (uint8_t)value[i]);
			}
		...
		*/
		...
		static long StrHexToLong(std::string const& input) {
			char * p;
			int32_t base = 16;
			long n = strtol(input.c_str(), &p, base);
			if (*p != 0) {
				return 0;
			}
			else {
