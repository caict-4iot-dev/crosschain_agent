...
std::string utils::GetPeerName(SocketHandle s) {
	sockaddr_in addr = { 0 };
...
utils::ISocketControl::ISocketControl() {}

utils::ISocketControl::~ISocketControl() {}

const utils::SocketHandle utils::Socket::INVALID_HANDLE = (utils::SocketHandle) - 1;
const int utils::Socket::ERR_VALUE = -1;

utils::Socket::Socket() {
	m_nType = Socket::SOCKET_TYPE_TCP;
	handle_ = INVALID_HANDLE;
	m_bWriteBlocked = false;
	m_bBlocking = true;
	m_bConnectBlocked = false;
	m_nSerailNumber = 0;
	m_pCustomControl = NULL;
}
...
utils::Socket::~Socket() {
	...
	if (m_pCustomControl) {
		delete m_pCustomControl;
		m_pCustomControl = NULL;
	}
}

bool utils::Socket::Attach(utils::SocketHandle nHandle, int nType, bool bBlocking) {
	...
	if (getsockname(handle_, (struct sockaddr *)&nAddr, &nAddrLen) == Socket::ERR_VALUE) {
		return false;
	}
	local_address_ = nAddr;

	nAddrLen = sizeof(nAddr);
	if (getpeername(handle_, (struct sockaddr *)&nAddr, &nAddrLen) == Socket::ERR_VALUE) {
		return false;
	}

	peer_address_ = nAddr;
	return SetBlocking(m_bBlocking);
}

bool utils::Socket::Detach() {
...
bool utils::Socket::Create(SocketType type, const InetAddress &address, bool resuse_addr) {
	assert(!IsValid());

	if (NULL != m_pCustomControl) {
		delete m_pCustomControl;
		m_pCustomControl = NULL;
	}
	...
	do {
		if (!SetReuse(resuse_addr) || bind(handle_, address.sock_addr(), address.size()) == ERR_VALUE) {
			break;
		}
...
bool utils::Socket::Close(const utils::SocketHandle &nHandle) {
	return closesocket(nHandle) != utils::Socket::ERR_VALUE;
}

bool utils::Socket::Bind(const utils::InetAddress &address) {
	do {
		SetReuse(true);
		if (bind(handle_, address.sock_addr(), address.size()) < 0) {
			break;
		}
		
		local_address_ = address;
		//local_address_.SetPort(0);
		return true;
	} while (false);
	return false;
}

bool utils::Socket::Listen(int nBackLog /* = SOMAXCONN */) {
	VALIDATE_ERROR_RETURN(!IsValid(), ERROR_NOT_READY, false);

	return listen(handle_, nBackLog) == 0;
}
...
bool utils::Socket::Connect(const InetAddress &server) {
	...
	m_bConnectBlocked = true;
	m_bWriteBlocked = true;
	peer_address_ = server;
	return false;
}
...
bool utils::Socket::SetBlocking(bool block) {
...
void utils::Socket::SetWriteBlocked(bool blocked) {
	m_bWriteBlocked = blocked;
}

bool utils::Socket::SetReuse(bool on) {
	int nReuse = on ? 1 : 0;
	return SetOption(SOL_SOCKET, SO_REUSEADDR, &nReuse, sizeof(nReuse));
}

bool utils::Socket::SetOption(int nSOL, int nOptName, const void *pValue, socklen_t nValueLen) {
	VALIDATE_ERROR_RETURN(!IsValid(), ERROR_NOT_READY, false);

	return setsockopt(handle_, nSOL, nOptName, (const char *)pValue, (int)nValueLen) != Socket::ERR_VALUE;
}
...
bool utils::Socket::Accept(utils::SocketHandle &nNewHandle, utils::InetAddress &nPeerAddress) {
	VALIDATE_ERROR_RETURN(!IsValid(), ERROR_NOT_READY, false);

	socklen_t nAddrLen = sizeof(*nPeerAddress.GetAddress());
	nNewHandle = accept(handle_, nPeerAddress.GetAddress(), &nAddrLen);

	return (Socket::INVALID_HANDLE != nNewHandle);
}

bool utils::Socket::Accept(Socket &new_socket) {
	SocketHandle new_handle = accept(handle_, NULL, 0);
...
uint32_t utils::Socket::GetSocketError() {
	int nErrorCode = ERROR_SUCCESS;
	socklen_t nErrorLen = sizeof(nErrorCode);

	getsockopt(handle_, SOL_SOCKET, SO_ERROR, (char *)&nErrorCode, &nErrorLen);

	return (uint32_t)nErrorCode;
}

void utils::Socket::SetBuffer(int32_t buf_len) {
	setsockopt(handle_, SOL_SOCKET, SO_SNDBUF, (const char *)&buf_len, sizeof(int));
	setsockopt(handle_, SOL_SOCKET, SO_RCVBUF, (const char *)&buf_len, sizeof(int));
}
