...
void MD5::md5bin(const void* dat, size_t len, unsigned char *out) {
		MD5_CTX c;
		MD5_Init(&c);
	...
	std::string MD5::GenerateMD5ToHex(const void* dat, size_t len) {
		unsigned char out[16];
		md5bin(dat, len, out);
		std::string res = "";
		for (size_t i = 0; i < 16; ++i) {
	...
	std::string MD5::GenerateMD5ToBin(const std::string &dat) {
		std::string md5;
		md5.resize(16);
		md5bin(dat.c_str(), dat.size(), (unsigned char *)md5.c_str());
		return md5;
	}

	std::string MD5::GenerateMD5ToHex(const std::string &dat) {
		return GenerateMD5ToHex(dat.c_str(), dat.length());
	}
	...
	std::string MD5::GenerateMD5Sum6(const std::string &dat) {
