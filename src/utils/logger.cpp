...
bool utils::LogWriter::Init(LogDest dest, const std::string &file_name, bool open_mode) {
	...
	else if(dest_ == LOG_DEST_PERFORMANCE) {
		file_ptr_ = stdout;
	}
...
std::string utils::LogWriter::GetLogPrefix(const LogLevel logLevel) {
	...
	case LOG_LEVEL_PERFORMANCE: return "PER";
	default:	return "";
	}
}
...
bool utils::Logger::Initialize(utils::LogDest log_dest, utils::LogLevel log_level, const std::string &file_name, bool open_mode) {
	...
	if (log_dest & LOG_DEST_FILE && !file_name.empty()) {
		...
		std::string per_file_name = file_name + "-per";
		...
		if (file_ext.size() > 0 && (file_ext.size() + 1) < file_name.size()) {
			...
			per_file_name = utils::String::Format("%s-per.%s", strSubPath.c_str(), file_ext.c_str());
		}
		...
		log_writers_[LOG_DEST_FILE_PER_ID].Init(LOG_DEST_FILE, per_file_name, open_mode);
	}
	...
	if (log_dest & LOG_DEST_PERFORMANCE) {
		log_writers_[LOG_DEST_PER_ID].Init(LOG_DEST_PERFORMANCE, "", false);
	}
...
int utils::Logger::LogStub(utils::LogLevel log_Level,
	...
	if (log_dest_ & LOG_DEST_FILE) {
		log_writers_[LOG_DEST_FILE_OUT_ID].Write(this, log_Level, time_string.c_str(),
			file_name.c_str(), funcName, lineNum,
			fmt, ap);
	}
	...
	if (log_Level >= LOG_LEVEL_WARN && log_Level < LOG_LEVEL_PERFORMANCE && log_dest_ & LOG_DEST_ERR) {
		log_writers_[LOG_DEST_ERR_ID].Write(this, log_Level, time_string.c_str(),
			file_name.c_str(), funcName, lineNum,
			fmt, ap);
	}

	if (log_Level >= LOG_LEVEL_PERFORMANCE && log_dest_ & LOG_DEST_PERFORMANCE) {
		log_writers_[LOG_DEST_PER_ID].Write(this, log_Level, time_string.c_str(),
			file_name.c_str(), funcName, lineNum,
			fmt, ap);
	}
