...
namespace utils {
	...
	public:
		...
		inline struct sockaddr *GetAddress() { return (struct sockaddr *)&addr_; }
	...
	std::string GetPeerName(SocketHandle s);
	...
	public:
		...
		// primary method
		virtual bool Initialize(const void *lpUserData) = 0;

		// action methods
		virtual bool Connect(int nTimeoutMillis) = 0;
		virtual bool Accept(int nTimeoutMillis) = 0;
		virtual int  Recv(void *pBuffer, const int nSize) = 0;
		virtual int  Send(const void *pBuffer, const int nSize) = 0;
		virtual int  RecvFrom(void *pBuffer, const int nSize, utils::InetAddress &nAddress) = 0;
		virtual int  SendTo(const void *pBuffer, const int nSize, const utils::InetAddress &nAddress) = 0;
		//virtual int  RecvFrom(void *pBuffer, const int nSize, utils::LowLevelAddress &nAddress) = 0;
		//virtual int  SendTo(const void *pBuffer, const int nSize, const utils::LowLevelAddress &nAddress) = 0;

		// for asynchronize socket
		virtual void OnConnect(int64_t nMicroTime) {}
	...
	public:
		...
		// initialize methods
		virtual bool Attach(SocketHandle nHandle, int nType, bool bBlocking);
		virtual bool Detach();
		bool Create(SocketType type, const InetAddress &address, bool resuse_addr);
		...
		bool SetReuse(bool on);

		virtual bool SetOption(int nSOL, int nOptName, const void *pValue, socklen_t nValueLen);
		...
		virtual bool Accept(SocketHandle &nNewHandle, utils::InetAddress &nPeerAddress);
		...
		inline bool  IsWriteBlocked() const { return m_bWriteBlocked; }
		inline bool IsConnectable() const { return Socket::SOCKET_TYPE_TCP == m_nType /*|| Socket::TYPE_SCTP == m_nType || Socket::TYPE_SSL_TCP == m_nType*/; }

		virtual uint32_t GetSocketError();

		void SetBuffer(int32_t buf_len);

		static bool Close(const SocketHandle &nHandle);
	...
	protected:
		...
		bool m_bBlocking;
		bool m_bWriteBlocked;
		int m_nSerailNumber;
		bool m_bConnectBlocked;
		ISocketControl *m_pCustomControl;
	};
	...
	class IAsyncVxSocketAcceptorNotify {
	public:
		IAsyncVxSocketAcceptorNotify() {};
		~IAsyncVxSocketAcceptorNotify() {};
		virtual void OnAccept(int64_t current_time) = 0;
		virtual void OnError(int64_t current_time) = 0;
	};
