...
bool utils::ThreadNewPool::InitThreadNum(std::size_t num, bool bindCpu) {
    if (num <= 0) {
...
std::size_t utils::ThreadNewPool::GetSize() {
    return m_threadVec.size();
}

void utils::ThreadNewPool::Wait() {
...
void utils::ThreadNewPool::Stop() {
    std::call_once(m_callFlag, [this] { terminate(); });
}

std::size_t utils::ThreadNewPool::GetTaskSize() {
    std::unique_lock<std::mutex> locker(m_mutex);
    return m_taskSize;
}

void utils::ThreadNewPool::JoinAll() {
...
void utils::ThreadNewPool::terminate() {
    std::unique_lock<std::mutex> locker(m_mutex);
    m_isStop = true;
...
void utils::ThreadNewPool::runTask() {
    while (true) {
...
void utils::ThreadNewPool::cleanTaskQueue() {
