...
namespace agent {
    Layer2Transaction::Layer2Transaction(){
        pb_transaction_.set_source_address(Configure::Instance().layer2_configure_.agent_address_);
        pb_transaction_.set_fee_limit(Configure::Instance().layer2_configure_.fee_limit_);
        pb_transaction_.set_gas_price(Configure::Instance().layer2_configure_.gas_price_);
    ...
    bool Layer2Transaction::AddTransactionOperation(const std::string& input,int64_t amount){
        ...
        paycoin->set_dest_address(Configure::Instance().layer2_configure_.cross_contract_);
    ...
    std::string Layer2Transaction::GetTransactionBlob(int64_t max_seq){
    ...
    bool Layer2Transaction::SetRandomNonce(int64_t nonce){
        if (nonce != 0){
    ...
    std::string Layer2Transaction::GetTransactionHash(){
